...
long |>
  group_by(test) |> 
  summarise(Mean = round(mean(time, na.rm=TRUE), 2), 
...
```{r}
(res.10 <- power_oneway_ancova(mu = c(0, sesoi.10), n_cov = 1, sd = pooledsd.10,
                     r2 = r.10^2, alpha_level = 0.05/6, beta_level = .2, 
                     round_up = TRUE, type = "exact"))
...
(ten <- sensitivity.10m |>
  ...
  annotate("segment", x = -0.11, xend = -0.11, y = 0, 
             yend = 30.8, colour = "green", size = 1.5, lineend = "round") + # sesoi
...
```{r}
(res.20 <- power_oneway_ancova(mu = c(0, sesoi.20), n_cov = 1, sd = pooledsd.20,
                     r2 = r.20^2, alpha_level = 0.05/6, beta_level = .2,
                     round_up = TRUE, type = "exact"))
...
(twenty <- sensitivity.20m |>
    ggplot(aes(target, power)) +
    annotate("segment", x = -0.3, xend = -0.25, y = 80, 
             yend = 79.9, colour = "red", size = 1.5, lineend = "round") + # 80% power
    annotate("segment", x = -0.25, xend = -0.25, y = 0, 
             yend = 79.9, colour = "red", size = 1.5, lineend = "round") + # critical es
    annotate("segment", x = -0.16, xend = -0.16, y = 0, 
             yend = 33.8, colour = "green", size = 1.5, lineend = "round") + # sesoi
    geom_line(linewidth = 2, lineend = "round") +
    coord_capped_cart(bottom='both', left='both') +
    scale_y_continuous(limits = c(0, 100),
                       breaks = seq(from = 0, to = 100, by = 10)) +
...
(res.40 <- power_oneway_ancova(mu = c(0, sesoi.40), n_cov = 1, sd = pooledsd.40,
                     r2 = r.20^2, alpha_level = 0.05/6, beta_level = .2,
                     round_up = TRUE, type = "exact"))
...
(forty <- sensitivity.40m |>
    ggplot(aes(target, power)) +
    annotate("segment", x = -0.40, xend = -0.29, y = 80, 
             yend = 80, colour = "red", size = 1.5, lineend = "round") + # 80% power
    annotate("segment", x = -0.29, xend = -0.29, y = 0, 
             yend = 80, colour = "red", size = 1.5, lineend = "round") + # critical es
    annotate("segment", x = -0.26, xend = -0.26, y = 0, 
             yend = 70.3, colour = "green", size = 1.5, lineend = "round") + # sesoi
    geom_line(linewidth = 2, lineend = "round") +
    coord_capped_cart(bottom='both', left='both') +
    scale_y_continuous(limits = c(0, 100),
                       breaks = seq(from = 0, to = 100, by = 10)) +
...
```{r}
# no covariate-outcome relationship available from Thurlow et al. (2024), so use 20 m (r.20)
(res.vmax <- power_oneway_ancova(mu = c(0, sesoi.vmax), n_cov = 1, sd = pooledsd.vmax,
                     r2 = r.20^2, alpha_level = 0.05/6, beta_level = .2,
                     round_up = TRUE, type = "exact"))
...
(vmax <- sensitivity.vmax |>
    ...
    annotate("segment", x = 0.675, xend = 0.675, y = 0, 
             yend = 80, colour = "red", size = 1.5, lineend = "round") + # critical es
    annotate("segment", x = 0.46, xend = 0.46, y = 0, 
             yend = 40, colour = "green", size = 1.5, lineend = "round") + # sesoi
    geom_line(linewidth = 2, lineend = "round") +
    coord_capped_cart(bottom='both', left='both') +
    scale_y_continuous(limits = c(0, 100),
                       breaks = seq(from = 0, to = 100, by = 10)) +
    ...
    labs(y = "Power (%)",  x = expression("VMax Target Change (m·s"^{-1}*")")) +
  easy_all_text_colour("black") +
...
Here, we can use the data directly from Datson et al. (2022) whereby a 2.8cm was reported as the meaningful change.
...
```{r}
(res.cmj <- power_oneway_ancova(mu = c(0, sesoi.cmj), n_cov = 1, sd = pooledsd.cmj,
                     r2 = r.cmj^2, alpha_level = 0.05/6, beta_level = .2,
                     round_up = TRUE, type = "exact"))
...
(cmj <- sensitivity.cmj |>
    ...
    annotate("segment", x = 10.3, xend = 10.3, y = 0, 
             yend = 80, colour = "red", size = 1.5, lineend = "round") + # critical es
    annotate("segment", x = 2.8, xend = 2.8, y = 0, 
             yend = 2.7, colour = "green", size = 1.5, lineend = "round") + # sesoi
    geom_line(linewidth = 2, lineend = "round") +
    coord_capped_cart(bottom='both', left='both') +
    scale_y_continuous(limits = c(0, 100),
                       breaks = seq(from = 0, to = 100, by = 10)) +
...
To determine other parameters needed for the sample size calculation, we can create dataframe with descriptive data from Tønnessen et al. (2013) (<https://doi.org/10.1123/ijspp.8.3.323>) and pre-post change scores from Thurlown et al. (2024).
...
```{r}
(res.mas <- power_oneway_ancova(mu = c(0, sesoi.mas), n_cov = 1, sd = pooledsd.mas,
                                r2 = r.yoyo^2, alpha_level = 0.05/6, beta_level = .2,
                                round_up = TRUE, type = "exact"))
...
(mas <- sensitivity.mas |>
    ...
    annotate("segment", x = 0.44, xend = 0.44, y = 0, 
             yend = 80, colour = "red", size = 1.5, lineend = "round") + # critical es
    annotate("segment", x = 0.13, xend = 0.13, y = 0, 
             yend = 5.2, colour = "green", size = 1.5, lineend = "round") + # sesoi
    geom_line(linewidth = 2, lineend = "round") +
    coord_capped_cart(bottom='both', left='both') +
    scale_y_continuous(limits = c(0, 100),
                       breaks = seq(from = 0, to = 100, by = 10)) +
    ...
    labs(y = "Power (%)",  x = expression("MAS Target Change (m·s"^{-1}*")")) +
  easy_all_text_colour("black") +
  theme(axis.title.x = element_text(vjust = -2)))
```
...
```{r}
(ten | twenty) / (forty | cmj) / (vmax | mas) & theme(text = element_text(size = 20))

#### Save figure
# Output format: SVG (scalable vector graphics). For PNG output, change extension to .png
ggsave(here("figures", "figure 2.svg"), dpi = 300, width = 30, height = 30, units = "cm")
```
...
```{r}
df10 <- data.frame(pre = c(1.85, 1.75, 1.75, 1.72, 1.74, 1.67, 1.69, 1.71,
                              1.82, 1.80, 1.78, 1.84, 1.75),
                  post = c(1.76, 1.69, 1.64, 1.70, 1.77, 1.56, 1.67, 1.65,
                               1.81, 1.64, 1.62, 1.60, 1.69))
cor(df10$pre, df10$post, method = c("pearson"))

df20 <- data.frame(pre = c(3.2, 2.98, 3.08, 3.03, 3.11, 2.92, 3.08, 2.95,
                           3.20, 3.11, 3.12, 3.12, 2.96),
                   post = c(3.21, 2.92, 2.96, 2.97, 3.03, 2.79, 2.95, 2.85,
                            3.13,2.88, 2.92, 2.85, 2.97))
cor(df20$pre, df10$post, method = c("pearson"))

dfcmj <- data.frame(pre = c(35.0, 34.0, 39.7, 36.8, 37.6, 33.8, 44.4, 45.9,
                            40.5, 41.2, 37.8, 39.8, 43.1),
                   post = c(35.6, 36.1, 40.4, 36.6, 39.8, 37.0, 43.9, 48.4,
                            41.5, 43.9, 38.4, 39.7, 45.6))
cor(dfcmj$pre, dfcmj$post, method = c("pearson"))

dfyy <- data.frame(pre = c(1880, 1600, 1200, 1920, 1440, 2600, 1800, 1920,
                           1880, 1640, 1800, 2200, 1880),
                    post = c(2360, 1760, 2400, 1760, 1880, 2760, 1840, 2080,
                             1720, 2360, 1800, 2360, 2760))
